...
echo "sls:::sls_ML_cluster(s=args[1],simpars=c(args[2],args[3],args[4],args[5]),cond=args[6])" >> zzz_ML.R

for((s = 1; s <= max_sims; s++))
do

echo "#!/bin/bash" > SLSjob$s
echo "#SBATCH --time=169:59:00" >> SLSjob$s #--time=229:59:00 #--time=95:59:00
#echo "module load R/3.3.1-foss-2016a" >> SLSjob$s
echo "module load R/3.4.4-foss-2018a-X11-20180131" >> SLSjob$s
echo "Rscript zzz_ML.R $s $lambda_M $mu_M $lambda_S $mu_S $cond" >> SLSjob$s
echo "rm SLSjob$s" >> SLSjob$s

#NEVER ASK FOR MORE THAN 9GB OF MEMORY!
sbatch --partition=regular --mem=9GB --job-name=ML$s --mail-type=FAIL,TIME_LIMIT --mail-user=glaudanno@gmail.com SLSjob$s
