...
#' @return result
#' @export
sls_sim <- function(pars1, age, N0 = 2, cond) {
  ...
  {#sim loop
    ...
    {#clade loop
      ...
      {#time loop
        ...
        if (total_rate > 0)
        {
          ...
          t <- t - deltaT
          if (t < tshift[shifts_already_occurred + 1] & t > 0) #if time surpasses the shift point
          {
            t <- tshift[shifts_already_occurred + 1] #time becomes the shift point
            ...
            shifts_already_occurred <- shifts_already_occurred + 1 #register that the shift occurred
          }else
          {
            if (deltaN > 0 & t > 0)
            {
      ...
      if (any(L[,4] == -1))
      {
        if (nrow(L) == 1) {time_points <- L[1,1]}
        else
        {
          time_points <- unlist(unname(
            sort(
              DDD:::L2brts(L, dropextinct = TRUE),
              decreasing = TRUE)
          ))
        }
    ...
    condLM1 <- sign(LM[,3]) != sign(shifted_id) #subclade does NOT start from here (M1)
    ...
    LS <- L.list[[2]]
    condM1cp <- sls:::check_survival(L = LM1, final_time = 0) #M1 survives from c to p
    condM2cp <- sls:::check_survival(L = LM2, final_time = 0) #M2 survives from c to p
    condM1cs <- sls:::check_survival(L = LM1cs, final_time = shift_time) #M1 survives from c to s
    condM2cs <- sls:::check_survival(L = LM2cs, final_time = shift_time) #M2 survives from c to s
    condS    <- sls:::check_survival(L = LS, final_time = 0) #S survives from s to p
...
#' @return result
#' @export
sls_sim0 <- function(pars1, age, soc, cond) {
  ...
  {#sim loop
    ...
    {
      L <- L.list[[clade]]
      if (clade == 1) #if in the main clade
      {
        L[L[,4] == td, 4] <- -1
        if (sls:::check_survival(L, final_time = 0) & all(L[,3] != 0)) #if L survives
        {
          if (sum(L[, 4] == -1) > 1) #if at least two species survive in the main clade
          {
      ...
      }else #if in the sub clade
      {
        if (sls:::check_survival(L) & all(L[,3] != 0)) #if L survives
        {
          if (sum(L[,4] == -1) > 1) #if at least two species survive in the sub clade clade
          {
            time_points <- unlist(unname(sort(DDD:::L2brts(L, dropextinct = TRUE), decreasing = TRUE)) )
            brts <- -sort(abs(as.numeric(time_points)), decreasing = TRUE)
            brts.list[[clade]] <- abs(brts)
