...
t_0 <- c(10, 4) #starting time for each clade
test <- DDD::dd_KI_sim(pars = c(pars[1], pars[2], Inf, pars[3], pars[4], Inf, t_0[2]), age = t_0[1], ddmodel = 1)
L <- test$L
L0 <- L[L[,5] == 0, 1:5]; dim(L0) <- c(sum(L[,5] == 0), 5); head(L0) #selecting only the main clade L[,5]==0
DDD::L2brts(L0) #why it doesn't work?
head(L) #how come that one of the two crown species belongs to the subclade if the shift is at t_0[2] = 4?
