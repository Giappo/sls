...
split_times_matrix <- function(times_matrix, N0 = 1){
  ...
  tb <- sls::times_matrix2t_coordinates(times_matrix = times_matrix)$tb
  ti <- sls::times_matrix2t_coordinates(times_matrix = times_matrix)$ti
...
#' @return result
#' @export
find_sim_function_name <- function(sim_function){

 fun_list <- ls("package:sls", pattern = "sim_custom")
 for (i in 1:length(fun_list)){
    if (all.equal(sim_function, get(fun_list[[i]])) == 1)
    {
      fun_name <- fun_list[[i]]
    }
 }
