...
test_likelihood_formula2 <- function(s, Nsims, lik_function = lik_custom, sim_function = sim_custom3){
  ...
  if (Nsims >= Nsims_threshold)
  {
    ...
    sim_function_name <- find_sim_function_name(sim_function = sim_function)

    # results_file <- paste0(getwd(),"//results//table4.xls")
    results_file <- paste0(getwd(),"//results//table_", sim_function_name,".xls")

    # if (!(results_file %in% list.files(paste0(getwd(),"/results"))))
    # {
    #   wb0   <- xlsx::createWorkbook(type="xls")
    #   sheet <- xlsx::createSheet(wb0, sheetName = "test")
    #   xlsx::saveWorkbook(wb0, results_file)
    # }

    sheet_name <- sheet_name0 <- paste0(data.sets[[s]],", N = ", Nsims)
